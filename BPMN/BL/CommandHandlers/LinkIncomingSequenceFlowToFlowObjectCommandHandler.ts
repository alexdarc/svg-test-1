...
class LinkIncomingSequenceFlowToFlowObjectCommandHandler
  implements ILinkIncomingSequenceFlowToFlowObjectCommandHandler{
  ...
  Handle(command: {
    ...
    let canLink = SequenceFlowConnectionRules.CanLinkIncomingSequenceFlow({
    ...
    this.flowObjectMapper.Update({
      ...
      update: new UpdateInstruction({
        pop: {
          IncomingIds: command.sequenceFlowId
        }
      })
    });
